(function () {
      ...
      try {
        return (
          sjcl.codec.base64.fromBits(
            RNCryptor.Encrypt(
              key, sjcl.codec.utf8String.toBits(txt)
            )
          )
        );
      } catch (e) {
      ...
      try {
        return (
          sjcl.codec.utf8String.fromBits(
            RNCryptor.Decrypt(
              key, sjcl.codec.base64.toBits(txt)
            )
          )
        );
      } catch (e) {
  ...
  var RNCryptor = {
    ...
    */
    KeyForPassword: function (password, salt) {
      ...
      return sjcl.misc.pbkdf2(password, salt, iterations, 32 * 8, hmacSHA1);
    }
    /*
    ...
    */
    ...
    Encrypt: function (password, plaintext, options) {
      options = options || {}
    ...
    */
    Decrypt: function (password, message, options) {
    options = options || {}
